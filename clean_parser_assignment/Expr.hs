...
value (Num n) dict = n
value (Var v) dict = if isJust(Dictionary.lookup v dict) then fromJust(Dictionary.lookup v dict) else error "variable is not defined."
...
value (Div e1 e2) dict = if ((value e2 dict) == 0) then error "Division by zero." else (div (value e1 dict) (value e2 dict)) 
